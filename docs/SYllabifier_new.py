...
[item for item in dir(tokenizers.syl) if not item.startswith('_')]

for syl in syl_examples.numpy():
  print(syl.decode('utf-8'))


encoded = tokenizers.syl.tokenize(syl_examples)

for row in encoded.to_list():
  print(row)


round_trip = tokenizers.syl.detokenize(encoded)
for line in round_trip.numpy():
  print(line.decode('utf-8'))

tokens = tokenizers.syl.lookup(encoded)

def tokenize_pairs(nosyl, syl):
    nosyl = tokenizers.nosyl.tokenize(nosyl)
    # Convert from ragged to dense, padding with zeros.
    nosyl = nosyl.to_tensor()

    syl = tokenizers.syl.tokenize(syl)
    # Convert from ragged to dense, padding with zeros.
    syl = syl.to_tensor()
    return nosyl, syl
...
transformer = Transformer(
    ...
    num_heads=num_heads,
    dff=dff,
    input_vocab_size=tokenizers.nosyl.get_vocab_size().numpy(),
    target_vocab_size=tokenizers.syl.get_vocab_size().numpy(),
    pe_input=1000,
    pe_target=1000,
    rate=dropout_rate)
...
class Translator(tf.Module):
  ...
  def __call__(self, sentence, max_length=20):
    ...
    sentence = self.tokenizers.nosyl.tokenize(sentence).to_tensor()
    
    encoder_input = sentence

    # as the target is english, the first token to the transformer should be the
    # english start token.
    start_end = self.tokenizers.syl.tokenize([''])[0]
    ...
    text = tokenizers.syl.detokenize(output)[0]  # shape: ()

    tokens = tokenizers.syl.lookup(output)[0]
    
    cleanup_text(reserved_tokens,text).numpy
...
for el in test_examples:
  print(reloaded(el[0]).numpy().decode('utf-8'))
  syllables = 0
  for char in el[0].numpy():
    if char == 'K':
      syllables += 1
  avgList.append(syllables)

print("avg syllables is: {}".format(np.average(avgList)))
